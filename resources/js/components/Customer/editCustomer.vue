            ...
            <div class="header">
                ...
                <div class="card col-md-12 hide-message" style="padding: 0 0 0 0; background-color: #117f2a; color: white;">
                    <div class="card-header">
                        Customer updated successfully, will redirect to the customer listing page in {{counter}} seconds ...
                    </div>
                </div>
            </div>
                ...
                <form @submit.prevent="updateData()">
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" v-model="form.email" class="form-control" id="inputEmail" disabled />
                        </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            const customerId = currentUrl.split("show/");
            
            axios.get(`/customer/edit/${customerId[1]}`)
                .then(response => {
                    if (response.data.statusCode === 200) {
                        const customerInfo = response.data.data;

                        // parse the data to the form
                        this.form.name = customerInfo.name;
                        this.form.email = customerInfo.email;
                        this.form.company_name = customerInfo.company_name;
                        this.form.company_address = customerInfo.company_address;
                        this.form.contact_number = customerInfo.contact_number;
                        this.form.phone_number = customerInfo.contact_number;
                        this.form.country = customerInfo.country;
                        if (customerInfo.country !== null && customerInfo.country !== '') {
                            this.getCountryTld(customerInfo.country);
                        } else {
                            this.getCountryTld('China');
                        }
                        this.form.reseller_name = customerInfo.reseller_name;
                    } else {
                        Swal.fire(
                            "500",
        ...
        methods: {
            getCountryTld(countryName) {
                ...
                // Find the matching country and return its top-level domain
                ...
                window.intlTelInput(input, {
                    ...
                    preferredCountries: this.selectedCountry,
                });
               
                const countryCodeString = $('.iti__selected-dial-code').text();
                if (countryCodeString && this.form.contact_number) {
                    this.form.contact_number = (this.form.contact_number).replace(countryCodeString, '');
                }
            },
            ...
            updateData() {
                ...
                const customerId2 = currentUrl2.split("show/");

                // API post request
                this.form.post(`/customer/update/${customerId2[1]}`)
                .then(response => {
                    if (response.data.statusCode === 200) {
